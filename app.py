...
def inject_css():
    """Inject custom CSS for a polished UI."""
    st.markdown("""
    <style>
    /* Page layout */
    ...
    .app-header h1 { font-size: 1.8rem; font-weight: 700; margin: 0 0 0.5rem; color: white; }
    .app-header p  { color: #94a3b8; margin: 0; font-size: 0.85rem; }
    .tech-badge {
    ...
    .src-name   { font-weight: 600; color: #1e40af; font-size: 0.85rem; }
    .score-high { color: #16a34a; font-weight: 600; font-size: 0.8rem; }
    .score-mid  { color: #d97706; font-weight: 600; font-size: 0.8rem; }
    .score-low  { color: #dc2626; font-weight: 600; font-size: 0.8rem; }
    .src-text   { color: #4b5563; font-size: 0.81rem; font-style: italic; margin-top: 0.4rem; line-height: 1.55; }
    ...
    .sb-metric-val   { font-size: 1.6rem; font-weight: 700; color: #1e40af; }
    .sb-metric-label { font-size: 0.7rem; color: #6b7280; text-transform: uppercase; letter-spacing: 0.06em; }
...
def render_sidebar():
    ...
    if stats["total_chunks"] == 0:
        ...
        st.sidebar.markdown(
            ...
            f'<div class="sb-metric-val">{stats["total_chunks"]}</div>'
            f'<div class="sb-metric-label">Indexed Chunks</div>'
            f'</div>',
            unsafe_allow_html=True,
        )
    ...
    st.sidebar.markdown(
        f'<span class="provider-badge">{LLM_PROVIDER}</span>'
        f'<span class="provider-badge">{model_name}</span><br>'
        f'<span class="provider-badge">{EMBEDDING_MODEL}</span>'
        f'<span class="provider-badge">ChromaDB</span>',
        unsafe_allow_html=True,
    )
...
def render_chat():
    ...
    if not st.session_state.chat_history:
        st.markdown(
            ...
            '<p>Upload documents using the sidebar, ingest them,<br>'
            'then ask questions grounded in your knowledge base.</p>'
            '</div>',
            unsafe_allow_html=True,
        )
    ...
    if st.session_state.last_sources:
        with st.expander(f"📎 {len(st.session_state.last_sources)} source(s) used in last response", expanded=False):
            for chunk in st.session_state.last_sources:
                ...
                st.markdown(
                    ...
                    f'&nbsp;&nbsp;<span class="{sc}">score: {chunk["score"]}</span>'
...
def main():
    ...
    st.markdown(
        ...
        "<h1>🔍 RAG Demo</h1>"
        "<p>Ask questions grounded in your documents — no hallucination.</p>"
